...
image: ""                               # optional; path under static/ used as this episode's social-share image (else site og-image.png)
...
- `internal/config/site.go`: `Site` struct mirroring `site.yml` (fields above, incl. optional `TwitterHandle`). `LoadSite(projectDir string) (*Site, error)` reads `<projectDir>/site.yml`, `yaml.Unmarshal`, then validates: required non-empty `Title`, `Author`, `OwnerEmail`, `Category`; `Language` defaults to `"en-us"` if empty; **`BaseURL` is parsed with `url.Parse` and must have scheme `http`/`https` and a non-empty host** — strip any trailing `/` (store normalized). Methods: `(*Site) AbsURL(p string) string` = `strings.TrimRight(BaseURL,"/") + "/" + strings.TrimLeft(p,"/")`; `(*Site) CoverURL() string` = `CoverArtURL` if set else `AbsURL("cover.png")` (square, for the podcast feed `<itunes:image>` + JSON-LD `image`); `(*Site) OGImageURL() string` = `AbsURL("og-image.png")` (1200×630 card, for `og:image`/`twitter:image`).
- `internal/config/episode.go`: `Episode` struct mirroring the episode YAML. **`Explicit` must be `*bool`** (`yaml:"explicit"`) so "omitted" is distinguishable from "explicitly false" — required for "episode overrides the site default" to work. Other optional string fields (`Slug`, `GUID`, `Image`) are plain `string` ("" ⇒ use the default). Plus `yaml:"-"` build-time fields `MP3Size, MP4Size int64`, `AudioDur, VideoDur int` (seconds). Methods: `(Episode) Pad() string` = `fmt.Sprintf("%04d", Number)`; `(Episode) EffectiveSlug() string` = `Slug` if set else `Pad()` (so the no-slug default URL is `episodes/0001.html`); `(Episode) PagePath() string` = `"episodes/" + EffectiveSlug() + ".html"`; `(Episode) MP3Path() string` = `"media/" + Pad() + ".mp3"` (media stays number-keyed — stable, matches the RSS enclosure); `(Episode) MP4Path() string` = `"media/" + Pad() + ".mp4"`; `(Episode) EffectiveGUID() string` = `GUID` if set else `"techgo-"+Pad()`; `(Episode) EffectiveExplicit(s *Site) bool` = `*Explicit` if `Explicit != nil` else `s.Explicit`; `(Episode) SocialImageURL(s *Site) string` = `s.AbsURL(Image)` if `Image != ""` else `s.OGImageURL()`. `LoadEpisodes(projectDir string) ([]Episode, error)`: glob `<projectDir>/data/*.yml`, unmarshal each, then **validate per episode**: `Number > 0`; `Slug` (if set) matches `^[a-z0-9][a-z0-9-]*$` and isn't a bare integer-with-leading-zeros that would collide with another episode's `Pad()`; `Title` non-empty; `SourceWav` and `SourceMp4` non-empty (file existence is checked in `media`/`build`, not here); `Image` (if set) is a relative path that exists under `<projectDir>/static/`. Error on duplicate `Number` **and on duplicate `EffectiveSlug()`**. Sort by `Number` **descending** (newest first → deterministic index + feed order). Errors should name the offending file.
...
- Helpers in `internal/render/jsonld.go`:
  ...
  - `site.OGImageURL()` = `site.AbsURL("og-image.png")` (the 1200×630 card from `techgo images`). `site.CoverURL()` = the square 3000×3000 cover (used for the podcast feed `<itunes:image>` and JSON-LD `image`). `Episode.SocialImageURL(s)` = `s.AbsURL(e.Image)` if `e.Image` set (Phase 1: optional `image:` field, a path under `static/` that `build` copies through) else `s.OGImageURL()`.
...
**`site.yml` gains** an optional `twitterHandle: "@techgo"` field (→ `twitter:site`); `Episode` gains an optional `image:` field (path under `static/`, used as that episode's social-share image). Both optional — if absent, the site-wide `og-image.png` is used everywhere.
...
- **Validation, early:** `LoadSite` checks `baseURL` is a real `http(s)://host` and strips trailing `/`; `LoadEpisodes` checks per-episode `number>0`, slug charset + uniqueness, non-empty title + source paths, and existing `image:` path; `CreateInvalidation` normalizes paths to start with `/`. Episode `explicit` is `*bool` so "inherit site default" vs "explicit override" are distinct.

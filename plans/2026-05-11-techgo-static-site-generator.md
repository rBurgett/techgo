...
```
`static/robots.txt` → `User-agent: *` / `Allow: /` / `Sitemap: <baseURL>/sitemap.xml` (a real sitemap is generated by `build` — see Phase 5; do **not** point this at `feed.rss`). The committed file carries a literal `baseURL` (and a comment saying so), but `techgo build` regenerates `robots.txt` from `site.baseURL` after copying `static/` (Phase 5, step 3), so the committed copy is just the editable fallback used by `techgo serve` on a not-yet-rebuilt dir. `static/site.webmanifest` → minimal name/short_name/theme_color `#1a1a1a`/background_color `#1a1a1a`/icons `[icon-512.png]`.
...
3. Copy `static/` recursively into `outputDir` (helper `copyDir` via `filepath.WalkDir` + `io.Copy`, preserving the `css/` subtree etc.). This also carries through any per-episode `image:` files that live under `static/`. **Then overwrite `<outputDir>/robots.txt`** with one generated from `site.BaseURL` (`User-agent: *` / `Allow: /` / `Sitemap: <baseURL>/sitemap.xml`) so the `Sitemap:` line can never drift from the configured base URL — the committed `static/robots.txt` stays as the editable fallback used by `techgo serve` on a not-yet-rebuilt dir.
